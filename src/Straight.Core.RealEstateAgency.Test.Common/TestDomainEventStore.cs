...
namespace Straight.Core.RealEstateAgency.Test.Common
{
    ...
    {
        private readonly Dictionary<Guid, IAggregator<IDomainEvent>> _memory = new Dictionary<Guid, IAggregator<IDomainEvent>>();

        public IReadOnlyDictionary<Guid, IAggregator<IDomainEvent>> Memory => ImmutableDictionary<Guid, IAggregator<IDomainEvent>>.Empty.AddRange(_memory);
